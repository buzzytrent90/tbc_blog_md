...
[multiple transactions](http://thebuildingcoder.typepad.com/blog/2012/11/temporary-transaction-trick-touchup.html) to
complete.

The dynamic model updater framework
[DMU](http://thebuildingcoder.typepad.com/blog/dmu),
...
He ran into this issue when trying to group elements in an updater, for instance for his
[dynamic insulation](http://www.youtube.com/watch?v=iXA264zZQ28) and dynamic
[arrangement of elements on a terrain surface](http://www.youtube.com/watch?v=rEj_lZ4uBcQ) tools.
The dynamic insulation group behaves almost like a custom entity in AutoCAD:
...
- The insulation tool needs to both delete and create new DetailCurves.
  ...
  To avoid ForbiddenForDynamicUpdateExceptions, the creation is placed in a completely new transaction.- The dynamic fence and arranging-items-on-terrain tools require creation of new FamilyInstances, which should be avoided in the IUpdater context, as stated in the Revit API help file RevitAPI.chm.- The error handling needs to be able to use FailureHandlingOptions, which again means that the add-in has to have full access to the transaction itself.
