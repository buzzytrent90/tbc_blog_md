...
```
FieldBuilder theVersion = schemaBuilder.AddSimpleField(VERSIONFIELD,
...
schemaBuilder.AddSimpleField(ZIPFIELD, typeof (bool))
  .SetDocumentation("Whether or not the stored data has been zipped.");
```
...
![Get adjacent rooms](img/get_rooms_1.png "Get adjacent rooms")
...
![Get adjacent rooms](img/get_rooms_2.png "Get adjacent rooms")
One way to handle this is to record the boundary information in a `Dictionary>`, where the key is the element id of the boundary segment, and the value is the elements that are bounded by that segment.
...
void ChangeDetailItemDrawOrder()
{
  ...
  if (activeDoc.IsFamilyDocument == true)
  {
    ...
    IEnumerable allViews0 = new FilteredElementCollector(activeDoc)
      .OfClass(typeof(View))
