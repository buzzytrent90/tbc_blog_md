...
- [Extensible storage – versioning best practices](#4)
...
![Cap](img/bachelor_masters_degree_cap.png)
#### Renaming a Family in a Project
\*\*Question:\*\* I have a question related to the `LoadFamilySymbol` method.
Here is the description:
```csharp
doc.LoadFamilySymbol( "test.rfa", "type1", out symbol1 );
doc.LoadFamilySymbol( "test.rfa", "type2", out symbol2 );
```
...
```csharp
document.LoadFamilySymbol( "test.rfa", "type1", out symbol1 );
symbol1.Family.Name = "NewName";
```
After this, the second load will create a new family. Renaming the symbol to the same name then doesn't work:
```csharp
document.LoadFamilySymbol( "test.rfa", "type2", out symbol2 );
```
...
- it is associated with 'test', but the first is ***not***
...
- load symbol S1 from family A → A.S1.
- rename S1's family to B → B.S1 is present in the project.
Now, to load A.S2, it might be sufficient to:
- temporarily rename A.rfa to B.rfa.
- load S2 from B.rfa → B.S1, B.S2 are present in the project.
...
Imagine that your app v1.0 requires 2 pieces of data: A and B, and your app v1.2 requires 3 pieces of data: A, B and C. Your v1.2 app reads storage and sees that there is no C. What does this mean? Was it due to old data, or C should not be present? Your next app v.1.3 requires new data format for B → B', may be with old data structure. Again, you have to deal with ambiguous environments; moreover, your app v.1.3 has to deal with 3 data formats: (A, B), (A, B, C) and (A, B', C).
...
- (A, B) → v1.0 processor that, for example, asks user about C and transfers data for the next processor.
- (A, B, C) → v.1.2 processor that converts B to B' using some algorithm.
- (A, B', C) → just use the data to do useful things.
...
```csharp
builder.AddMapField(
"strings", typeof( string ), typeof( string ) );
builder.AddMapField(
"ints", typeof( string ), typeof( int ) );
FieldBuilder field = builder.AddMapField(
"doubles", typeof( string ), typeof( double ) );
field.SetUnitType( UnitType.UT_Custom );
builder.AddMapField( "bools",
