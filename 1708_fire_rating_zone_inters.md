...
#### 3D Solid Extrusion Creation and Intersection Task
...
![Fire rating zone intersection and subdivision](img/fire_rating_zone_intersection.png)
...
In the end I swapped out the RvtClipper `Result` `Execute` method to return a `List` and substituted the `doc.Selection` based component for two element parameters that I pass in when calling it: the boundary element and whichever element I am currently testing for an intersection as I iterate through them.
...
All the best.
Thanks.
#### Soffit Subdivision Sample Code
```csharp
using System;
...
using ClipperLib;
using Polygon = System.Collections.Generic.List;
using Polygons = System.Collections.Generic.List>;
...
{
#region Get Soffits
List Soffits = new FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Floors).ToElements().Where(m => !(m is ElementType)).ToList();
#endregion
//Subdivide
...
foreach (List LC in LLC)
{
List LCSorted = new List();
try
{
...
List MaybeBoundary = new FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Floors).ToElements().Where(m => !(m is ElementType)).ToList();
...
string RefString = "";
List CArray = com.Execute(revit, ref RefString, BoundryElement as Floor, Soffit as Floor);
...
List Curves = SortCurvesContiguous(CAL, false);
List NewCurveEnds = new List();
#endregion
...
public List SortCurvesContiguous(
IList curves,
bool debug_output)
{
int n = curves.Count;
...
![Fire rating zone intersection model](img/fire_rating_zone_intersection_model.png)
The result is a bunch of new floor elements:
![Fire rating zone intersection result](img/fire_rating_zone_intersection_result.png)
