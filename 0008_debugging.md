...
For completeness sake, and to ensure that we are really all on the same page, I suggest we start from scratch with a brand new Revit add-in project. That means that we will be going through the same steps as those described in the DevTV presentation and webcast recording mentioned in the
[getting started](http://thebuildingcoder.typepad.com/blog/2008/08/getting-started.html)
post. Here is the short version of the description to set up a new Revit add-in:
...
- Do not forget to set the 'Copy Local' flag to False. You do so by opening the properties of RevitAPI.dll. See
  [The SDK Samples Solution](http://thebuildingcoder.typepad.com/blog/2008/09/the-sdk-samples.html)
