...
We are still on the
[Pantagruel](http://www.classicsailing.de), currently
sailing from Norway to the west coast of Sweden.
Here is a view from Preikestolen down into Lysefjord, where we went for a hike before leaving the Stavanger area:

![The view from Preikestolen down into Lysefjord](file:////j/photo/jeremy/2010/2010-09-06_norway/andreas/dsc06491.jpg)
Here are some of us on deck the first day:
![First hours on deck](file:////j/photo/jeremy/2010/2010-09-06_norway/andreas/dsc06507.jpg)

That was before getting out onto the open sea and into higher winds, causing us to feel a lot less chirpy:

![Slightly stronger winds and waves](file:////j/photo/jeremy/2010/2010-09-06_norway/andreas/dsc06512.jpg)

The weather has been pleasant all the time, but the wind is very weak now.
...
![Sunset](file:////j/photo/jeremy/2010/2010-09-06_norway/andreas/dsc06524.jpg)

The view towards the east was like a
[Rothko](http://en.wikipedia.org/wiki/Mark_Rothko) painting, a study in blue:

![Study in blue](file:////j/photo/jeremy/2010/2010-09-06_norway/andreas/dsc06527.jpg)

It was sunny all the time in Norway.
...
Later... almost there was a big exaggeration.
...
Tuesday morning our aim was to leave as early as possible and get over to the islands north of Gothenburg in Sweden, but due to a pure easterly wind we ended up further south on the Danish island of
[Læsø](http://en.wikipedia.org/wiki/L%C3%A6s%C3%B8),
where I am sitting watching the sunset right now.

You can follow our exact route on the
[Pantagruel tracker](http://share.findmespot.com/shared/faces/viewspots.jsp?glId=0fQEu0hRkYX5FQpeMDEj8hms84EuXOsRl).

Returning once again to Revit programming, we talked about the Ribbon API and
[simulating a textbox label](http://thebuildingcoder.typepad.com/blog/2010/09/simulating-a-ribbon-textbox-label.html) the other day.
...
**Question:** I have been having problems correctly displaying icons and images in my Revit add-in.
...
1. Copy the original Ribbon sample project with all its support files.- Rename Reset.png to something else, and replace it with [\_\_icon\_16x16\_org.png](zip/__icon_16x16_org.png).- Rename RectangleWall.png to some other name, and replace it with [\_\_icon\_16x16\_resaved.png](zip/__icon_16x16_resaved.png).- Start Revit and check icons.

The first image is the one causing a problem.
The second one was reopened in Paint and simply resaved.
Both images look like this, normally:

![Original icon](img/ribbon_icon_resolution_1.png)

But in Revit, the resulting panel looks like this:

![Different icons in Revit ribbon](img/ribbon_icon_resolution_2.png)

As you can see, the top image above is not showing correctly, while the lower one looks as expected.

Looking at the image attributes using Paint > Image > Attributes..., you can see that the resolution or DPI (dot per inch) settings differ.
The one that does not work shows up like this:

![Problematic icon resolution setting](img/ribbon_icon_resolution_3.png)

If the file is resaved, it works fine, and the image attributes then look like this:

![Unproblematic icon resolution setting](img/ribbon_icon_resolution_4.png)

Apparently, once the image is saved in Paint, the DPI information is stripped out.
The file becomes slightly smaller and the resolution changes from 72 to 96 dpi.

Is this an expected behaviour of the image specification?
...
**Answer:** From the technical point of view, the underlying Windows Presentation Foundation technology WPF used in the ribbon postulates 96 dpi as the default resolution setting.
