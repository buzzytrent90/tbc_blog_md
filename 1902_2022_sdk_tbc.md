...
tags: ['csharp', 'elements', 'family', 'filtering', 'geometry', 'levels', 'parameters', 'references', 'revit-api', 'sheets', 'vbnet', 'walls']
source_file: "1902_2022_sdk_tbc.md"
original_url: "https://thebuildingcoder.typepad.com/blog/1902_2022_sdk_tbc.html"
---
...
The first is trivial – adapt its `\*.addin` file appropriately and copy it to the Revit `AddIns` folder.
...
![RvtSamples 2022](img/rvtsamples_2022.png "RvtSamples 2022")
...
The behaviour remained the same – a parameter with `ParameterType.Text` would have `GetSpecTypeId()` == `SpecTypeId.Number`.
...
- ParameterType.Number → SpecTypeId.Number
...
- ParameterType.Length → SpecTypeId.Length
...
- tag.GetTaggedLocalElement() → tag.GetTaggedLocalElements()
- newTag.TaggedElementId → newTag.GetTaggedElementIds()
...
![The Building Coder samples 2022](img/tbc_samples_2022.png "The Building Coder samples 2022")
