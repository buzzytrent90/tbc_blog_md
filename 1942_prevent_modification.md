...
I looked at
the video [BIMedge – Admin User to Lock out Worksets](https://youtu.be/0KS2nwHWZRQ) that
shows the Workset stuff that you mean – the thing is, that I can easily change my account into "Admin" and so every other user could do it like that (change username, etc.).
...
![RevitLookup maximise button](img/revitlookup_maximise.png "RevitLookup maximise button")
Many thanks to
...
![RevitLookupWpf](img/revitlookupwpf.jpg "RevitLookupWpf")
...
> I just performed an Internet search for ¶ and learned that it is named [Pilcrow](https://en.wikipedia.org/wiki/Pilcrow).
