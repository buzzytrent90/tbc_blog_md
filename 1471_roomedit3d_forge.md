...
- 3-legged oauth? (for a360 access?) – yes, using the NPM package for the [DM Data Management API](https://developer.autodesk.com/en/docs/data/v2/overview).
- 2-legged oauth? (for box access?) – actually 3-legged too, using the [BOX npm package](https://github.com/Developer-Autodesk/data.management-nodejs-integration.box#packages-used).
- A360 file download and upload – yes, again using the NPM package.
- box file download and upload – yes, using box NPM package
...
- `FORGE_CALLBACK_URL` – that’s the name we use on Dev Portal, cf. [this config.js sample](https://github.com/augustogoncalves/forge-3leg.nodejs-template/blob/master/server/config.js#L24)
- `PORT` – use 3000, as per common NodeJS practice, cf. [sample here](https://github.com/augustogoncalves/forge-3leg.nodejs-template/blob/master/server/server.js#L45)
...
![LmvNavTest default 3D and selected 2D secondary view](img/lmvnavtest.png)
It even links the elements in the two views so that anything selected in one is highlighted in the other:
![LmvNavTest selected element synchronisation across views](img/lmvnavtest_select.png)
