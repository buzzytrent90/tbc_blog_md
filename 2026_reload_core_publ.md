...
![Year of the Dragon](img/2024_dragon_year.jpg "Year of the Dragon")
...
![ProjectInfo C4R publish views data](img/c4a_views_projectinfo.png "ProjectInfo C4R publish views data")
...
public static void PublishedViews(Document d)
{
  ...
  var viewSheetSetIds = entity.Get>(field);
  var viewSheetSets = viewSheetSetIds.Select(id => d.GetElement(new ElementId(id))).Cast();
  var views = viewSheetSets.SelectMany(viewSheetSet => viewSheetSet.Views.Cast());
...
[how to assign category / graphics style `` to edges of titleblock via API](https://stackoverflow.com/questions/77942445/how-to-assign-category-graphics-style-invisible-lines-to-edges-of-titleblo):
\*\*Question:\*\*
...
I found that I can achieve this by editing the titleblock family manually and assigning the category `` to the edges of the sheet, but I can't manage to do this programmatically.
![Line properties](img/line_properties.png "Line properties")
...
I used RevitLookup to view the data of the corresponding category, which appears to also be called `` and to have the `Id` -2000064.
...
var lineStyle = graphicsStyles.FirstOrDefault(x => x.Name == "");
