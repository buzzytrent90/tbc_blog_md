...
We discussed the Revit
[unit handling](http://thebuildingcoder.typepad.com/blog/2008/09/units.html)
...
Revit includes a comprehensive internal unit handling library. One use of this is to convert and display the internal Revit database units to whatever units the user prefers to see in the user interface.
...
[format unit strings](http://thebuildingcoder.typepad.com/blog/2008/11/formatting-units-strings.html)
...
For completeness' sake, we also mention another area related to units that we touched on briefly, the exploration of
...
[service-oriented architecture](http://thebuildingcoder.typepad.com/blog/2009/08/serviceoriented-architecture.html) or
...
[UnitConversion source code](UnitConversion.zip).
...
#### Using the Revit API to Perform Unit Conversions for You

When using the Revit API and needing to store values into your document, Revit expects you to provide those values in the units of measure that Revit uses internally.
This has been discussed previously in
[this article](http://thebuildingcoder.typepad.com/blog/2008/09/units.html).
...
Normally, when using the Revit graphical interface, you set your Project Units to the units of measure in which you are working, type in your data in those units, and Revit takes care of the rest.
...
The key piece of the puzzle is to use the 'SetValueString' method instead of the 'Set' method on a parameter.
...
For example, Revit uses Decimal Feet as the internal unit of measure for Length.
...
Further, SetValueString allows you to override the current units of measure for the project by including unit symbols in your string.
...
As another example, having your project units for Length set to meters and using SetValueString with the value: 2' 3" (including the single and double quotes) will store the value 2.25 into the parameter.
...
A parallel reverse function called AsValueString also exists in the Revit API.
...
Using this knowledge, it is possible to write methods which can be used to help solve the unit conversion problem much more easily.
...
This has been accomplished in the UnitConversion sample, whose
[source code](UnitConversion.zip)
...
![UnitConversion](img/UnitConversion.png)

We can see near the top of this form that for CFM or cubic feet per minute units, the scale factor (multiplier) we need to store our value into Revit is 0.01666666666666.
...
For the mathematically inclined, this scale factor equates to 1/60, which tells us the internal units Revit uses for air flow are Cubic-Feet-per-Second.
...
We can also see near the bottom of the form that when we store a value of 1.0 CFM, the internal value will be 0.01666666.
...
[Jeremy's manual approach](http://thebuildingcoder.typepad.com/blog/2008/09/units.html):
...
Further, we can actually have Revit convert that value to a very different set of units of measure.
...
If you explore the sample code you will probably gain some more insights into how Revit works with parameters and units of measure.
