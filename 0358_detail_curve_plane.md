...
The day before yesterday, I mentioned that the geometry curve used to define a detail curve
[must lie in the plane](http://thebuildingcoder.typepad.com/blog/2010/05/detail-curve-must-be-in-plane.html)
...
1. How can I determine the plane equation or transformation of the current view or sketch plane in order to transform the curve to it as required?- Could you please provide a working sample of the NewDetailCurve method actually creating a curve element in Revit 2011?
...
The second question caused a surprise for me, because it shows how important the new behaviour actually is.
...
My initial response was to say: Sure, simply run The Building Coder sample command
[CmdDetailCurves](http://thebuildingcoder.typepad.com/blog/2009/09/detail-lines.html), which I recently
[ported to 2011](http://thebuildingcoder.typepad.com/blog/2010/03/porting-the-building-coder-samples-to-revit-2011.html).
...
So obviously I have been providing a curve that does not lie in the plane all along.
...
![Planar detail curves in Revit 2011](img/detail_curves_2011.png)

Come to think of it and comparing this image with the one in the
[original post](http://thebuildingcoder.typepad.com/blog/2009/09/detail-lines.html),
...
Here is an updated
[version 2011.0.65.1](zip/bc_11_65_1.zip)
of The Building Coder sample code including the fixed implementation of the external command CmdDetailCurves.
I changed the
[version numbering system](http://en.wikipedia.org/wiki/Software_versioning)
again from the one I
[described last](http://thebuildingcoder.typepad.com/blog/2010/03/porting-the-building-coder-samples-to-revit-2011.html):
...
To project a point orthogonally onto a plane, you can simply pick the closest point on the plane to the given point.
...
```csharp
Point3d Plane::closestPointTo( const Point3d & p ) const
{
...
The signed distance from a point to a plane can be easily determined by calculating the dot product between the point and the plane normal 'm\_n' and subtracting the distance of the plane from the origin 'm\_d':
```csharp
inline double Plane::signedDistanceTo( const Point3d & p ) const
{
  return m\_n.dotProduct( p.asVector() ) - m\_d;
}
```