...
- [Example – standard approach](#3.3)
- [Example – Revit.Async approach](#3.4)
...
Actually, we can achieve all the above logic by making use of task-based asynchronous pattern (TAP) which is generally known as `Task` in .NET.
...
![Revit API external event](img/revit_async_external_event.png "Revit API external event")

Revit API external event

![Revit.Async](img/revit_async_external_event.png "Revit.Async")
...
Use the `IGenericExternalEventHandler` interface instead.
...
- `AsyncGenericExternalEventHandler` → use to execute asynchronous logic
- `SyncGenericExternalEventHandler` → use to execute synchronize logic
```csharp
[Transaction(TransactionMode.Manual)]
public class MyRevitCommand : IExternalCommand
{
...
SyncGenericExternalEventHandler
{
