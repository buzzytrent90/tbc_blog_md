...
[modeless door lister](http://thebuildingcoder.typepad.com/blog/2010/12/modeless-door-lister-and-deleter.html) created
by
[Agnius Vasiliauskas](http://coding-experiments.blogspot.com) as
...
You should probably read Arnošt's suggestions as a follow-up on his explanation of
...
I am looking at the code of the application and you can probably guess I do not like it that much (although our PM is rather excited about it).
The sample demonstrates too many rather dangerous techniques for my taste.

1. It uses a direct connection to Revit's main window handle (probably safe, but risky).- It uses transactions from others than the main thread (as far as I can tell – I haven't actually run it).- The Idling event handler is not very useful here – other events would be far more effective (DocumentOpened, ViewActivated, DocumentChanged, etc.)- Events should be only subscribed to by a command if they are also unsubscribed in the same Execute method (not very common); that is why we recommend subscribing to events from the Application.OnStartup method.
...
1. An application creates a modeless dialog either from OnStartup, a command or an event, depending on what the dialog is designed to do.- When the dialog is created and is designed to communicate back to Revit at any time, the application also subscribes to the Idling event. (The subscription must happen before the original command, event or OnStartup ends and should not be done from the dialog itself, for it is running in another thread already.)- The Idling event handler only does something when some signalled state is On, otherwise it does nothing.- When the end user requests an action via the dialog (a button pressed, or something), the dialog collects the data it needs to get to Revit and sets the signalled state to On.- Next time Idling is raised, the handler notices the signalled state and calls the dialog to do what it needs to do.
...
By the way, a slightly cleaner non-generic example of using the Idling event to handle messages received from a modeless dialogue attached to the main Revit window is given by my
...
[generic pattern](http://thebuildingcoder.typepad.com/blog/2010/11/pattern-for-semi-asynchronous-idling-api-access.html) for this kind of semi-asynchronous Idling API access.