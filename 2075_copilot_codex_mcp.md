...
![MCP GitHub stars](img/2025-04-22_mcp_github_stars.png "MCP GitHub stars")

![A2A interaction](img/2025-04-22_a2a_interaction.png "A2A interaction")
...
- Ribbon integration
– On startup, it adds an “HVAC System Tools” tab with buttons for
...
– Before duplicating, it walks your existing system and builds a little graph of “nodes” (every pipe segment or fitting) and their “connectors” (endpoints), capturing for each:
...
– In a single TransactionGroup it:
...
– In a second transaction it revisits every new node, finds on each its closest connector on a neighboring new element (using a tiny distance‑threshold test) and calls the Revit API “ConnectTo” to wire them back up in exactly the same topology as the original.
