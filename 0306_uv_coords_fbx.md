...
**Question:** How can I access the texture data on a Revit model using the Revit API?
...
I am currently using the FBX SDK to retrieve the texture file name from the texture library.
...
**Question:** I love the Building Coder.
But there's a topic you haven't covered – do you know if it's possible to export textures coordinates from within the Revit SDK?
We've seen the UVArray class, but can't work out how to get an UVArray from a mesh or other geometry object.

**Answer:** The Revit API has some ability to extract information about material assets from an RVT file.
...
However the mesh vertices accessible through the API are the XYZ coordinates of each vertex.
...
In the FBX SDK, the ImportScene sample program might be helpful.
You can use it like this:

- Create a wall in Revit Architecture.- Export an FBX file using File > Export > 3DS Max FBX.- Run the FBX SDK sample 'ImportScene' on the generated FBX file.- Import the FBX file into 3DS Max.

3DS Max reads and displays the Revit model including all its Mesh, UV information, Material and Texture information.
This means that the FBX file has all the required information.
The support file assetlibrary\_base.fbx probably also includes some library material and texture information.
