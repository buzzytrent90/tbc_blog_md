...
public static class RibbonController
{
  ...
  {
    RootGrid = VisualUtils.FindVisualParent(ComponentManager.Ribbon, "rootGrid");
    if (RootGrid is null) throw new InvalidOperationException("Cannot find root grid in Revit UI");

    InternalToolPanel = VisualUtils.FindVisualChild(RootGrid, string.Empty);
...
![OptionsBar](img/rk_optionsbar_2.jpg "OptionsBar")
...
![OptionsBar](img/rk_optionsbar_3.jpg "OptionsBar")
...
![OptionsBar](img/rk_optionsbar_4.jpg "OptionsBar")
...
```
  private OptionsViewModel SetupOptionsBar()
  {
    ...
    {
      Offset = 0,
      Constraints = Document.EnumerateInstances(BuiltInCategory.OST_Levels).Select(level => level.Name).ToArray()
    };
...
```
  private Wall PickWall()
  {
    ...
    return reference.ElementId.ToElement(Document);
  }
```
...
![OptionsBar](img/rk_optionsbar_5.gif "OptionsBar")
#### Conclusion
...
> [Method Draw](https://editor.method.ac) is a simple [open source](https://github.com/methodofaction/Method-Draw) vector drawing application. Method Draw was forked from [SVG-Edit](https://github.com/SVG-Edit/svgedit) several years ago with the goal of improving and modernizing the interface.
> At this time (2021), the author ([Mark MacKay](http://method.ac/writing)) is working on improving stability and improving the codebase, which contains a lot of legacy practices. The goal is to create a vector editor suitable for simple graphic design tasks.
