...
We already discussed quite a few aspects of the
[add-in manifest files](http://thebuildingcoder.typepad.com/blog/2010/04/addin-manifest-and-guidize.html),
including:

- A simple example of creating one for my first Revit 2011 plug-in, the
  [pipe to conduit converter](http://thebuildingcoder.typepad.com/blog/2010/05/pipe-to-conduit-converter.html).- The [RevitAddInUtility](http://thebuildingcoder.typepad.com/blog/2010/04/revitaddinutility.html) which
    can be used to read and write add-in manifest files.- My [Guidize utility](http://thebuildingcoder.typepad.com/blog/2010/04/addin-manifest-and-guidize.html) which
      can populate the add-in manifest ClientId GUID.- A [Guidize update](http://thebuildingcoder.typepad.com/blog/2010/04/addin-manifest-and-guidize.html) which
        also handles the AddInId tag.- The [visibility mode](http://thebuildingcoder.typepad.com/blog/2010/05/addin-visibility-mode.html) which
          controls which in which Revit editor contexts an external command add-in appears.- How to handle [external applications for multiple flavours](http://thebuildingcoder.typepad.com/blog/2010/06/addin-applications-for-multiple-revit-products.html) of Revit.

Here is a new question on the location of the add-in manifest files:

**Question:** Is there a way to make Revit look for add-in manifest files in any other locations than the All Users and current user's AppData folders?
I have a client that would like to place them on a server rather than distribute them across many machines.

**Answer:** As you say, add-in manifest files are automatically picked up and processed by Revit simply by placing them in
[specific folders](http://thebuildingcoder.typepad.com/blog/2010/04/addin-manifest-and-guidize.html#1) in
...
Due to inherent security settings in the Microsoft .NET Framework, .NET applications cannot access registry settings or the file system without permission when applications (.dlls) are loaded from a network location.
...
1. C:\Windows\Microsoft.NET\Framework\v2.0.50727\caspol.exe -addgroup 1.2 -url \\someserver\someshare\somefolder\\* FullTrust- C:\Windows\Microsoft.NET\Framework\v2.0.50727\caspol.exe -listgroups- C:\Windows\Microsoft.NET\Framework\v2.0.50727\caspol.exe -remgroup 1.2.3

Revit itself does not provide any option in Revit to modify the add-in manifest location.
...
**Question:** Is the creation of a ribbon panel with some UI widgets on it by an external application stored on a network drive possible?
...
It seems the error is associated with making the .PNG file into a bitmap for the ribbon panel.
...
**Answer:** I suspect this is not a Revit issue, but rather related to the .NET framework, similarly to above.
...
Here is some more recent information on the
[code access security policy (caspol.exe) GUI utility](http://brandonpotter.wordpress.com/2010/01/23/code-access-security-policy-caspol-exe-gui-utility),
