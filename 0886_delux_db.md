...
The method is designed solve a somewhat unique problem, but the resulting solution has interesting possibilities for other projects of all sizes.
Here it is:
...
This short paper describes a novel method to extend database functionality for a "smart" loading and updating process in a very large Revit project.
It builds upon the white paper on
[database integration](http://thebuildingcoder.typepad.com/blog/2009/01/database-integration.html) by
...
**Addendum:** I continue to imagine more ways of extending the method.
...
This will require us to make some changes in how we store information in the database, but could work.
...
They are provided at no cost via the
[class catalogue](http://au.autodesk.com/?nd=classes)
on the AU website, available to anyone with an AU account.

If you don't have an account, you can
[sign up for free](http://au.autodesk.com/?nd=new_registration) at
any time.

Here are
[all the details](http://au.autodesk.com/?nd=au_blaug&rss_data_id=47818).

#### Setting a Structural Framing Tag Leader Arrowhead

In a recent AEC DevBlog article, Saikat Bhattacharya demonstrates how to
[set the leader arrowhead for a structural framing tag](http://adndevblog.typepad.com/aec/2013/01/setting-the-leader-arrowhead-for-a-structural-framing-tag-using-revit-api.html).

His sample code first retrieves all arrowhead definitions, which are ElementType instances whose family name is "Arrowhead".
They are obtained using a filtered element collector and a parameter filter testing the built-in parameter ALL\_MODEL\_FAMILY\_NAME for that value.
Once the desired arrowhead is found, the IndependentTag built-in parameter LEADER\_ARROWHEAD is set to its element id.

#### Using DMU to Track Changes to a Custom Project Parameter

Another of Saikat's articles discusses the implementation of a
[dynamic model updater for project parameters](http://adndevblog.typepad.com/aec/2013/01/dynamic-model-updater-with-project-parameters.html).
...
For this purpose, Rudolf Honke recommends using [log4net](http://logging.apache.org/log4net/).
It comes equipped with a very helpful [tutorial](http://www.codeproject.com/Articles/140911/log4net-Tutorial) as well.