...
[Revit 2013 MEP API and the external services framework](http://thebuildingcoder.typepad.com/blog/2012/05/the-revit-2013-mep-api-and-external-services.html).

In preparation for my Revit MEP API presentation at the upcoming
...
I obviously had to change the Revit API assembly references, which forces an update of the .NET framework version to 4.0.
...
Debugging the external application OnStartup method revealed that since I was running and testing it in a one-box flavour of Revit, the product type was not 'MEP' but simply 'Revit'.
...
![AdnRme 2013 ribbon panel](img/AdnRme_2013_panel.png)
...
![AdnRme 2013 about box](img/AdnRme_2013_about.png)
...
Several descriptions have been made publicly available in the past, though, for instance in the materials for my
[Autodesk University 2011](http://au.autodesk.com/?nd=e2011) virtual class
...
Before that, I already published the code here on the blog for
[Revit MEP 2012](http://thebuildingcoder.typepad.com/blog/2011/08/mep-sample-code-for-revit-2012.html) and
...
- HVAC air terminal analysis and sizing.- Hierarchical display of an electrical system.

It also demonstrates some Revit and .NET API aspects not directly related to the MEP API, such as:

- External application implementation.- About box, automatically populated using .NET assembly properties.- Progress bar.

AdnRme implements an external application to define a custom user interface to access the sample commands, as you can see from the ribbon panel shown above.
...
![AdnRme 2013 progress bar](img/AdnRme_2013_progress_bar.png)
...
- Placement and sizing of air distribution ducts and terminals.- Analysis and verification of results.

In a little more detail, the workflow might look like this:

- Manually lay out air terminals.- Run heating and cooling load analysis.- Based on load analysis, assign flow to terminals.- Based on terminal flow, size air terminals.- Validate design; a common check figure uses air flow per surface area.

The MEP HVAC sample was originally implemented for Revit 2008 and based on room elements, since spaces had not yet been introduced back then.
It was modified for RME 2009 to use spaces.

It implements the following four commands to support the HVAC engineering workflow described above.
The first three support the actual workflow itself, the fourth cleans up and resets the data manipulated by them:

- Assign flow to terminals.- Change air terminal size.- Calculate and display flow density as colour fill to verify design by air flow per surface area.- Reset demo.

This workflow is exercised in the following simple office building sample model with a place holder duct system in place:

![HVAC sample model](img/AdnRme_2013_hvac_model.png)

Here it is in plan view:

![HVAC sample model plan view](img/AdnRme_2013_hvac_plan.png)

The progress of the steps is displayed by four schedules set up in the sample model:

![HVAC sample schedules](img/AdnRme_2013_hvac_schedules.png)

Initially, all the 51 diffusers are the same:

![HVAC initial quantities](img/AdnRme_2013_hvac_initial_quantities.png)

The 33 spaces have calculated and specified supply air flows defined, but no actual flow:

![HVAC spaces](img/AdnRme_2013_hvac_spaces.png)
...
![HVAC air terminal sizes](img/AdnRme_2013_hvac_air_terminal_sizes.png)

Their flow and actual supply air flow is still zero:

![HVAC terminal flow](img/AdnRme_2013_hvac_terminal_flow.png)

Executing the commands in the proper sequence fixes all this:

- Assign flow to terminals: calculate the required flow per terminal by dividing the required flow for the space by the number of terminals, and assigning the resulting value to each terminal via its Flow parameter.- Change air terminal size: for each diffuser type, determine its minimum and maximum flow. Select the type that ensures that the actual flow lies between these two.- Calculate and display flow density as colour fill to verify design by air flow per surface area: calculate the CFM per SF using the built-in parameters ROOM\_ACTUAL\_SUPPLY\_AIRFLOW\_PARAM and ROOM\_AREA and set the "CFM per SF" parameter accordingly.

The schedules are all updated, and the predefined colour fill displays and enables visual verification of the results:

![HVAC colour fill](img/AdnRme_2013_hvac_colour_fill.png)
...
![Electrical hierarchy tree view](img/AdnRme_2013_electrical_hierarchy.png)

It actually determines and displays the hierarchical structure of the electrical systems in the model in several different ways.
Its focus is to:

- Traverse the electrical system- Reproduce the system browser data structure in a tree view- Determine and display the complete connection hierarchy in a tree view

The standard RME system browser displays electrical components in a three-level flat list, and the complete hierarchical structure of the connection tree is not immediately apparent.
...
- CmdElectricalConnectors- CmdElectricalSystemBrowser- CmdUnhostedElements
...
The form displaying the full electrical system hierarchical structure is shown above.
...
Clicking on a panel node in the left-hand list redisplays the tree on the right, with all the tree nodes except the selected panel or device collapsed.
Here we select the panel H-2:

![Electrical hierarchy at H-2](img/AdnRme_2013_electrical_hierarchy_H-2.png)

Here, transformer T1 is selected instead:

![Electrical hierarchy at T1](img/AdnRme_2013_electrical_hierarchy_T1.png)

Being modeless, the form remains visible and can be navigated after the command has terminated.
...
Note, however, that updates to the model will not be reflected in the dialogue until the command is re-executed. A sample model elec\_project.rvt is provided.
Also note that this was originally implemented a long time ago and does not take all of Arnošt's recent advice on interaction with a modeless dialogue and
[avoiding too much use of the Idling event](http://thebuildingcoder.typepad.com/blog/2012/01/avoid-idling.html) into
account.
...
- Obsolete pre-connector approach- Three-tier system browser structure- Find un-hosted elements
...
![System browser tree](img/AdnRme_2013_system_browser.png)
...
![Unhosted elements message box](img/AdnRme_2013_unhosted.png)

As said, I documented some other aspects of this sample in the last few years' Revit MEP API classes at Autodesk University and the AEC DevCamp two years back, so if you really want to know still more, you might be lucky there.
I think this just about covers it, though, actually more than enough.

Here is
[adn\_rme\_2013.zip](file:///C:/a/lib/revit/2013/adn_rme_2013.zip) containing
...
The US Army Corps of Engineers ASACE has released version 1 of their
[Revit Architecture, Structure, MEP templates](https://cadbim.usace.army.mil).
...
- [Federal Business Opportunities](https://www.fbo.gov)- [Federal procurement bidding opportunity search](http://cbd-net.com/index.php/doc/home)- [USACE FY12 Acquisition Forecast – Military Construction](http://www.usace.army.mil/Portals/2/docs/MILCON/AcquisitionForecastMilitaryConstructionMAR2012.pdf)