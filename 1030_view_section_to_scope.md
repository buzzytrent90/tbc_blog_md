---
post_number: "1030"
title: "Set View Section Box to Match Scope Box Updated for Revit 2014"
slug: "view_section_to_scope"
author: "Jeremy Tammik"
tags: ['csharp', 'elements', 'geometry', 'python', 'references', 'revit-api', 'selection', 'transactions', 'views']
source_file: "1030_view_section_to_scope.htm"
original_url: "https://thebuildingcoder.typepad.com/blog/1030_view_section_to_scope.html"
---

### Set View Section Box to Match Scope Box Updated for Revit 2014

A year ago, we looked at
[setting the view section box to match its scope box](http://thebuildingcoder.typepad.com/blog/2012/08/set-view-section-box-to-match-scope-box.html),
in particular:

- How to retrieve the exact geometric location, size and orientation of the scope box.
- How to set the exact geometric location, size and orientation of the 3D view section box.

Dan Rumery of
[BVN Donovan Hill](http://www.bvn.com.au) recently
submitted a
[comment](http://thebuildingcoder.typepad.com/blog/2012/08/set-view-section-box-to-match-scope-box.html?cid=6a00e553e168978833019aff6c2563970d#comment-6a00e553e168978833019aff6c2563970d) to
point out an error in the code presented there:
...
Here is
[SetSectionBox2014.zip](zip/SetSectionBox2014.zip) containing
the full new source code, Visual Studio solution and add-in manifest.
...
An example that exhibits the bug in question is if you move the scope box away from the origin and / or rotate the scope box around the Z axis so that it no longer aligns to the X and Y axes.
The fixed version of the code handles this case perfectly.
...
Here is the raw Python file
[bbox\_from\_scope\_box.py](zip/bbox_from_scope_box.py.txt).

You can run this code in the Revit Python Shell.
...
It requires a 3D view active and exactly one scope box selected in it, and assumes the first element of the selection set is a scope box element.

Please refer to the
[original post](http://thebuildingcoder.typepad.com/blog/2012/08/set-view-section-box-to-match-scope-box.html) for
